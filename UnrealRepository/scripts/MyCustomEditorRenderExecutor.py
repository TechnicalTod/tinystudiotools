    ...
    def execute_delayed(self, memory_queue):
        ...
        if num_jobs > 0:
            ...
            for job in jobs:
                ...
                for setting in settings:
                    if 'MoviePipelineOutputSetting' in setting.get_class().get_path_name():
                        ...
                        if not check_path.startswith('CUSTOM'):
                            ...
                            # Set the path based on the show
                            ...
                            seq_path += '/'
                            seq_path = f'Y:/{project_name}/episodes/' + seq_path
                            ...
                            new_seq_path = f'{seq_path}/work/LO/unreal/renders/{last_part}'
                ...
                if len(parts) >= 2:
                    # Get the second directory from the end
                    result = parts[-3]

                #perforceAutomation.push_all_pending_changes(result)
                perforceAutomation.push_specific_changes(result, assetpath)
                perforceAutomation.pull_unreal_changes_to_perforce(f'{project_name}_Shared') # Pull onto X:drive - This is the shared on x for remote machines
                ...
                args = (
                    f'"X:/PerforceWorkspaces/{project_name}_Shared/{project_name}/{project_name}.uproject" '
