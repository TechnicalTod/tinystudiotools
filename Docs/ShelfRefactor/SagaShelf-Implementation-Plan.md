# Maya Shelf Button System - Implementation Plan

## Phase 1: Tool Discovery System

1. **Create Auto-Discovery Module**

   - Create new file `MayaRepository/2023/scripts/autoDiscovery.py`
   - Implement `SagaToolDiscovery` class with the following methods:
     - `discover_all_tools()` - Main entry point for scanning directories
     - `_scan_category(category, dir_path)` - Scan a specific tool category directory
     - `_scan_python_file(category, py_file)` - Extract tool metadata from a Python file
     - `_extract_tool_configs(file_content)` - Parse docstrings for tool metadata
   - Add support for scanning all directories ending with "Tools"
   - Implement error handling for import failures
   - Add logging for discovery process

2. **Create Metadata Parser**
   - Implement regex-based parser for extracting JSON from docstrings
   - Create JSON schema validation for tool metadata
   - Handle edge cases like malformed JSON and missing fields
   - Implement default values for optional metadata fields

## Phase 2: Tool Resolver Implementation

1. **Create Tool Resolver**

   - Implement dynamic module importing system
   - Add support for resolving tool functions by name
   - Create caching mechanism for imported modules
   - Add error handling for missing or invalid functions

2. **Create Function Wrapper**

   - Implement wrapper for tool functions to add error handling
   - Add logging for tool execution
   - Create consistent error reporting for tool failures
   - Support for passing parameters to tool functions

## Phase 3: Shelf Builder Implementation

1. **Refactor Shelf Builder**

   - Create new version of `buildSagaShelf.py` that uses auto-discovery
   - Implement shelf factory pattern for creating buttons
   - Add support for organizing tools by category
   - Create consistent error handling for button creation

2. **Implement Button Creation Logic**

   - Create methods for building different button types
   - Add support for menu creation based on tool categories
   - Implement spacer creation for visual organization
   - Add support for tool icons and tooltips

## Phase 4: Tool Metadata Implementation

1. **Update Existing Tools**

   - Add `SAGA_TOOL_CONFIG` metadata to key tools:
     - Start with `modellingTools/cubinate.py` as an example
     - Update `assetTools/createAssetDirUI.py`
     - Update other frequently used tools
   - Test metadata extraction and validation
   - Verify tool discovery and button creation

2. **Create Helper Modules**

   - Implement utility functions for common operations
   - Create helper modules for MEL command wrappers
   - Add modules for UI utilities
   - Create modules for scene utilities

## Phase 5: Testing & Integration

1. **Unit Testing**

   - Test tool discovery in isolation
   - Test metadata parsing with various input formats
   - Test tool resolver with mock modules
   - Test shelf builder with mock tools

2. **Integration Testing**

   - Test full workflow from discovery to shelf creation
   - Verify all existing tools are properly discovered
   - Test error handling with problematic tool definitions
   - Measure performance impact on Maya startup

3. **User Testing**
   - Have artists test the new shelf system
   - Gather feedback on organization and usability
   - Identify any missing tools or functionality
   - Make adjustments based on feedback

## Phase 6: Documentation & Deployment

1. **Developer Documentation**

   - Document the `SAGA_TOOL_CONFIG` format and options
   - Create examples of properly formatted tool metadata
   - Document the auto-discovery process
   - Create guidelines for adding new tools

2. **User Documentation**

   - Update any user-facing documentation
   - Document the new shelf organization
   - Create quick reference for available tools
   - Document any changes in tool behavior

3. **Deployment**
   - Deploy to test environment
   - Verify functionality in production-like setting
   - Roll out to production
   - Monitor for any issues

## Implementation Steps for Auto-Discovery

### Step 1: Create the Auto-Discovery Module

Create a new file `MayaRepository/2023/scripts/autoDiscovery.py` with the following content:

```python
"""
Tool discovery system for Maya shelf buttons.
Automatically finds and catalogs tools based on docstring metadata.
"""

import os
import importlib
import inspect
import json
import re
from pathlib import Path
from typing import Dict, List, Any

class SagaToolDiscovery:
    """Auto-discover Maya tools from script directories"""

    def __init__(self, scripts_path: str):
        self.scripts_path = Path(scripts_path)
        self.discovered_tools = {}
        self.categories = {}

    def discover_all_tools(self):
        """Scan all tool directories and discover tools"""
        print("🔍 Discovering SAGA tools...")

        # Look for directories ending with 'Tools'
        for dir_path in self.scripts_path.iterdir():
            if (dir_path.is_dir() and
                dir_path.name.endswith('Tools') and
                not dir_path.name.startswith('_')):

                category = dir_path.name
                print(f"📁 Scanning {category}...")
                self._scan_category(category, dir_path)

        # Also scan genTools (special case)
        gen_tools_path = self.scripts_path / 'genTools'
        if gen_tools_path.exists():
            print(f"📁 Scanning genTools...")
            self._scan_category('genTools', gen_tools_path)

        print(f"✅ Discovered {len(self.discovered_tools)} tools across {len(self.categories)} categories")
        return self.discovered_tools

    def _scan_category(self, category: str, dir_path: Path):
        """Scan a single category directory"""
        if category not in self.categories:
            self.categories[category] = []

        for py_file in dir_path.glob('*.py'):
            if py_file.name.startswith('_'):
                continue

            try:
                self._scan_python_file(category, py_file)
            except Exception as e:
                print(f"⚠️  Error scanning {py_file}: {e}")

    def _scan_python_file(self, category: str, py_file: Path):
        """Scan a Python file for tool configurations"""
        module_name = f"{category}.{py_file.stem}"

        try:
            # Read the file content to look for SAGA_TOOL_CONFIG
            with open(py_file, 'r', encoding='utf-8') as f:
                content = f.read()

            # Look for SAGA_TOOL_CONFIG in docstrings
            configs = self._extract_tool_configs(content)

            if configs:
                # Import the module to get actual functions
                module = importlib.import_module(module_name)

                for config in configs:
                    entry_point = config.get('entry_point', 'launch')

                    if hasattr(module, entry_point):
                        func = getattr(module, entry_point)
                        tool_id = f"{category}.{py_file.stem}.{entry_point}"

                        self.discovered_tools[tool_id] = {
                            'function': func,
                            'module_name': module_name,
                            'file_path': str(py_file),
                            'category': category,
                            'config': config
                        }

                        self.categories[category].append(tool_id)
                        print(f"  ✓ Found: {config.get('label', tool_id)}")

        except ImportError as e:
            print(f"⚠️  Could not import {module_name}: {e}")
        except Exception as e:
            print(f"⚠️  Error processing {py_file}: {e}")

    def _extract_tool_configs(self, file_content: str) -> List[Dict]:
        """Extract SAGA_TOOL_CONFIG from file content"""
        configs = []

        # Look for SAGA_TOOL_CONFIG: followed by JSON
        pattern = r'SAGA_TOOL_CONFIG:\s*(\{[^}]*\})'
        matches = re.findall(pattern, file_content, re.DOTALL)

        for match in matches:
            try:
                # Clean up the JSON (remove comments, fix quotes)
                cleaned_json = self._clean_json_string(match)
                config = json.loads(cleaned_json)
                configs.append(config)
            except json.JSONDecodeError as e:
                print(f"⚠️  Invalid JSON in SAGA_TOOL_CONFIG: {e}")

        return configs

    def _clean_json_string(self, json_str: str) -> str:
        """Clean up JSON string for parsing"""
        # Remove Python-style comments
        lines = json_str.split('\n')
        cleaned_lines = []

        for line in lines:
            # Remove comments after //
            if '//' in line:
                line = line[:line.index('//')]
            cleaned_lines.append(line)

        return '\n'.join(cleaned_lines)

    def get_tools_by_category(self, category: str) -> List[str]:
        """Get all tool IDs for a category"""
        return self.categories.get(category, [])

    def get_shelf_tools(self) -> List[Dict]:
        """Get tools that should appear on the shelf"""
        shelf_tools = []

        for tool_id, tool_data in self.discovered_tools.items():
            config = tool_data['config']
            if config.get('shelf_button', True):  # Default to True
                shelf_tools.append({
                    'id': tool_id,
                    'function': tool_data['function'],
                    'config': config,
                    'category': tool_data['category']
                })

        return shelf_tools
```

### Step 2: Update the Shelf Builder

Create a new version of `buildSagaShelf.py` that uses the auto-discovery system:

```python
"""
Dynamic shelf builder for Maya that uses auto-discovery to find and organize tools.
"""
import shelfCreate
import mayaFilePaths
from autoDiscovery import SagaToolDiscovery

def makeShelfAndButtons():
    """Build shelf automatically from discovered tools"""

    # Discover all tools
    discovery = SagaToolDiscovery(mayaFilePaths.mayaScriptsPath)
    discovery.discover_all_tools()

    # Create shelf
    iconPath = mayaFilePaths.mayaShelfIconPath
    shelf = shelfCreate.ShelfTool('SAGA_SHELF', iconPath)
    shelf.createShelf()

    # Get tools that should appear on shelf
    shelf_tools = discovery.get_shelf_tools()

    # Group tools by category
    categorized_tools = {}
    for tool in shelf_tools:
        category = tool['category']
        if category not in categorized_tools:
            categorized_tools[category] = []
        categorized_tools[category].append(tool)

    # Add spacer
    safe_create_button(shelf, lambda: shelf.addShelfSpacer(iconImage='spacer.png'))

    # Create buttons for each category
    for category, tools in categorized_tools.items():
        if len(tools) == 1:
            # Single tool - create simple button
            tool = tools[0]
            config = tool['config']
            button = safe_create_button(shelf, lambda t=tool: shelf.shelfButton(
                t['function'],
                config.get('icon', 'default.png'),
                config.get('tooltip', config.get('label', 'Tool'))
            ))

        else:
            # Multiple tools - create group button with menu
            category_config = get_category_config(category)
            primary_tool = get_primary_tool(tools)

            button = safe_create_button(shelf, lambda: shelf.shelfButton(
                primary_tool['function'] if primary_tool else lambda: None,
                category_config.get('icon', f'{category.lower()}.png'),
                category_config.get('tooltip', f'{category} Tools')
            ))

            if button:
                # Add menu items for each tool
                for tool in tools:
                    config = tool['config']
                    shelf.addMenu(button, config.get('label', 'Tool'), tool['function'])

    print("🎉 Saga shelf built successfully!")

def safe_create_button(shelf, create_func):
    """Safely create shelf button with error handling"""
    try:
        return create_func()
    except Exception as e:
        print(f"⚠️  Failed to create shelf button: {e}")
        return None

def get_category_config(category: str) -> dict:
    """Get configuration for a tool category"""
    configs = {
        'assetTools': {'icon': 'asset.png', 'tooltip': 'Asset Tools'},
        'modellingTools': {'icon': 'modelling.png', 'tooltip': 'Modelling Tools'},
        'riggingTools': {'icon': 'rigging.png', 'tooltip': 'Rigging Tools'},
        'shadingTools': {'icon': 'shaderTools.png', 'tooltip': 'Shading Tools'},
        'textureTools': {'icon': 'textureTools.png', 'tooltip': 'Texture Tools'},
        'unrealTools': {'icon': 'unrealTools.png', 'tooltip': 'Unreal Tools'},
        'techvisTools': {'icon': 'techvis.png', 'tooltip': 'Techvis Tools'},
        'genTools': {'icon': 'genTools.png', 'tooltip': 'General Tools'},
        'cameraTools': {'icon': 'cameraTools.png', 'tooltip': 'Camera Tools'}
    }
    return configs.get(category, {'icon': 'default.png', 'tooltip': f'{category} Tools'})

def get_primary_tool(tools: list) -> dict:
    """Get the primary tool for a category (first one marked as primary or first tool)"""
    for tool in tools:
        if tool['config'].get('primary', False):
            return tool
    return tools[0] if tools else None
```
