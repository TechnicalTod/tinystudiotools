...
# Set up colored logging with UTF-8 encoding
...
# Force UTF-8 encoding for the handler's stream
...
class LaunchController:
    ...
    def prepare_launch_config(self, app_name: str, version: str, show: str) -> LaunchConfig:
        ...
        # Ensure directories exist for any path-based environment variables
        for key, value in env_vars.items():
            if "DIR" in key and not key.endswith("PATH"):
                try:
                    path = Path(value)
                    if not path.exists() and ":" in str(path):  # Looks like a file path
                        path.mkdir(parents=True, exist_ok=True)
                        logger.debug(f"Created directory for {key}: {path}")
                except Exception as e:
                    logger.debug(f"Could not create directory for {key}: {e}")
...
if __name__ == "__main__":
    ...
    try:
        launch_config = controller.prepare_launch_config("maya", "2023", "TEST_SHOW")
        print(f"\nPrepared launch config for Maya 2023:")
