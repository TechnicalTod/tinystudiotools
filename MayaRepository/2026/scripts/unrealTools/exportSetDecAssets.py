...
class MainWindow(QtWidgets.QWidget):
    ...
    def add(self, shapeList=None):
        ...
        setDecGroupFolderBasePath = "{}/{}/03_Production/Assets/SETDEC".format(
            SHOWDRIVE, selectedShow
        )
        setDecGroupFolderName = (
            setDecGroupFolderBasePath + "/" + self.setDecGroupComboBox.currentText() + "/"
        )
        ...
        if badShapeList:
            ...
            self.warningPopup(
                "You are trying to load a Set Dec item with no connected shapes, this item has been removed from the list: {}".format(
                    badShapeListString
                )
            )
        ...
        for row_number, shape in enumerate(shapes):
            ...
            self.tableWidget.setItem(
                init_row_number + row_number, 0, QtWidgets.QTableWidgetItem(shape.name())
            )
            ...
            try:
                ...
                if len(variantList) > 1:
                    with open(
                        "{}/qComboBoxMultiItemYellow.qss".format(mayaFilePaths.styleSheetFilepath),
                        "r",
                    ) as fh:
            ...
            try:
                for versionFolder in os.listdir(
                    setDecGroupFolderName + splitShapeName + "/" + variantComboBox.currentText()
                ):
                ...
                with open(
                    "{}/qComboBoxMultiItemGreen.qss".format(mayaFilePaths.styleSheetFilepath), "r"
                ) as fh:
            ...
            self.getLatestVersionNumber(
                init_row_number + row_number, versionComboBox, newVersionComboBox
            )

            # set combobox update linking
            variantComboBox.currentTextChanged.connect(
                lambda index, row=init_row_number + row_number, column=1: self.updateVerNumOnVarChange(
                    row
                )
            )
            versionComboBox.currentTextChanged.connect(
                lambda row=init_row_number + row_number, versionComboBox=versionComboBox, newVersionComboBox=newVersionComboBox: self.getLatestVersionNumber(
                    row, versionComboBox, newVersionComboBox
                )
            )
    ...
    def updateVerNumOnVarChange(self, row):
        ...
        try:
            for versionFolder in os.listdir(
                setDecGroupFolderName + splitSetDecObjectName + "/" + setDecVariant
            ):
                versionList.append(versionFolder)
        ...
        print(
            "Variant changed to {}/{} finding latest versions".format(
                splitSetDecObjectName, setDecVariant
            )
        )
    ...
    def renameSelectedVariants(self):
        ...
        newVariantName, ok = input_dialog.getText(
    ...
    def getDuplicatesInList(self):
        ...
        duplicateShapeNames = set(
            [name for name in setDecAssetList if setDecAssetList.count(name) > 1]
        )
        ...
        for row in reversed(sorted(rowList)):
            ...
            try:
                if setDecObject.published.get():
                    ...
                    publishedSetDecObjectShaders = pm.ls(
                        pm.listConnections(publishedSetDecShadingGroups), materials=True
                    )
                    for shader in publishedSetDecObjectShaders:
                        ...
                        try:
                            ...
                            if pm.objExists(publishedShaderObject):
                                ...
                                pm.sets(
                                    publishedShadingGroups[0], e=True, forceElement=faceAssignments
                                )
                        except:
                            print(
    ...
    def collectListForPublish(self):
        ...
        if duplicateShapeNames:
            ...
            self.warningPopup(
                "Duplicate shape names found in list: {}, please fix or remove from list before publishing".format(
                    duplicateShapeNames
                )
            )
            return
        if publishedShapeNames:
            ...
            self.warningPopup(
                "Published shapes found in list: {}, please unpublish or remove from list before publishing".format(
                    publishedShapeNames
                )
            )
            return
            ...
            for row in range(init_row_number):
                ...
                if pm.objExists(setDecObject):
                    ...
                    existingShaders, textureBasePath, pubTexturedict = self.publishSetDecTextures(
                        setDecObject, splitSetDecObjectName, setDecVariantName, setDecNewVersion
                    )
                    self.publishSetDec(
                        setDecObject,
    ...
    def publishSetDecTextures(
        self, setDecObject, splitSetDecObjectName, setDecVariantName, setDecNewVersion
    ):
        selectedShow = self.showComboBox.currentText()
        setDecGroupFolderBasePath = "{}/{}/03_Production/Assets/SETDEC/".format(
            SHOWDRIVE, selectedShow
        ...
        for shader in set(shaderList):
            ...
            for parameter in parameterList.get("USDPreviewMaterial"):
                try:
                    ...
                    fileNode = pm.listConnections(
                        "{}.{}".format(shader, mayaParameterName), source=True, destination=False
                    )
                    ...
                    if fileNode:
                        ...
                        textureList = [
                            os.path.join(texturePath, tex)
                            for tex in os.listdir(texturePath)
                            if tex.endswith(".png") and fileNameSplit in tex
                        ]
        ...
        for shader in pubTexturedict:
            ...
            for parameterName in pubTexturedict[shader]:
                ...
                mayaParameterName = (
                    parameterList.get("USDPreviewMaterial").get(parameterName).get("mayaParameter")
                )
    ...
    def publishSetDec(
        ...
        importedNodes = pm.importFile(
            mayaFullPath, returnNewNodes=True, mergeNamespacesOnClash=False
        )
