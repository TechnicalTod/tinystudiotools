...
def makeShelfAndButtons():
    ...
    try:
        versionUpCurrentFileButton = slapFactoryShelf.shelfButton(
            versionUpCurrentFileCMD, "VersionUp.png", "Version up open Maya file"
        )
    except Exception as e:
        import traceback
    ...
    try:
        centerPivotButton = slapFactoryShelf.shelfButton(
            centerPivotCMD, "centerPivot.png", "Center Pivot"
        )
        slapFactoryShelf.addMenu(
            centerPivotButton, "Create Locator at selection", createLocAtPivotCMD
        )
    except Exception as e:
        import traceback
    ...
    try:
        freezeTransformsButton = slapFactoryShelf.shelfButton(
            freezeTransformsCMD, "freezeTransforms.png", "Freeze Transforms"
        )
    except Exception as e:
        import traceback
    ...
    try:
        deleteHistoryButton = slapFactoryShelf.shelfButton(
            deleteHistoryCMD, "deleteHistory.png", "Delete History"
        )
        slapFactoryShelf.addMenu(
            deleteHistoryButton, "Delete all unconnected shapes", deleteUnconnectedShapesCMD
        )
    except Exception as e:
        import traceback
    ...
    try:
        moveToOriginButton = slapFactoryShelf.shelfButton(
            moveToOriginCMD, "moveToOrigin.png", "Move To Origin"
        )
    except Exception as e:
        import traceback
    ...
    try:
        movePivotBAButton = slapFactoryShelf.shelfButton(
            movePivotBACMD, "movePivotBA.png", "Move Pivot B to A"
        )
    except Exception as e:
        import traceback
    ...
    try:
        movePivotToOriginButton = slapFactoryShelf.shelfButton(
            movePivotToOriginCMD, "movePivotToOrigin.png", "Move Pivot To Origin"
        )
    except Exception as e:
        import traceback
    ...
    try:
        modellingButton = slapFactoryShelf.shelfButton(
            combineObjectsCMD, "modelling.png", "Modelling Tools"
        )
        ...
        slapFactoryShelf.addMenu(
            modellingButton, "Select same Obj by polycount", selectByPolycountCMD
        )
        slapFactoryShelf.addMenu(modellingButton, "Separate selected faces", detachComponentsCMD)
        slapFactoryShelf.addMenu(
            modellingButton, "Dulicate selected faces", detachDuplicateComponentsCMD
        )
        slapFactoryShelf.addMenu(
            modellingButton, "Separate shapes by shading groups", separateByShadingGroupsCMD
        )
        ...
        slapFactoryShelf.addMenu(
            modellingButton, "Convert Megascans Assets", convertMegascansAssetsCMD
        )
    ...
    try:
        ...
        slapFactoryShelf.addMenu(
            riggingButton, "Export puppet for Skeletal Mesh", exportPuppetForSkeletalMeshCMD
        )
        ...
        slapFactoryShelf.addMenu(
            riggingButton, "Load Advanced Skelton Shelf", loadAdvancedSkeletonCMD
        )
    ...
    try:
        textureToolsButton = slapFactoryShelf.shelfButton(
            convertTextureUICMD, "textureTools.png", "Texture Tools"
        )
    ...
    try:
        shaderToolsButton = slapFactoryShelf.shelfButton(
            blankCMD, "shaderTools.png", "Shader Tools"
        )
        slapFactoryShelf.addMenu(shaderToolsButton, "", blankCMD)
        slapFactoryShelf.addMenuSpacer(shaderToolsButton, label="Commands")
        slapFactoryShelf.addMenu(
            shaderToolsButton, "Rename shader to shape name", renameShaderToShapeNameCMD
        )
        slapFactoryShelf.addMenu(
            shaderToolsButton, "Apply random vertex colour to selection", randomVertColCMD
        )
        slapFactoryShelf.addMenu(
            shaderToolsButton, "Toggle vertex color display", toggleVertexColorDisplayCMD
        )
        slapFactoryShelf.addMenu(
            shaderToolsButton, "Create Blinn for substance export", createBlinnPerShapeCMD
        )
        slapFactoryShelf.addMenu(
            shaderToolsButton, "Copy shader to objects", copyShaderToObjectsCMD
        )
        slapFactoryShelf.addMenuSpacer(shaderToolsButton, label="Tools")
        slapFactoryShelf.addMenu(
            shaderToolsButton,
            "Build shader network from Substance",
            buildShaderNetworkFromSubstanceCMD,
        )
    except Exception as e:
        import traceback
    ...
    try:
        ...
        slapFactoryShelf.addMenu(
            genToolsButton, "Delete All Display Layers", deleteDisplayLayersCMD
        )
    ...
    try:
        importExportMayaButton = slapFactoryShelf.shelfButton(
            blankCMD, "copyPaste.png", "Copy/Paste Selection"
        )
    ...
    try:
        snapshotToolsButton = slapFactoryShelf.shelfButton(
            takeSnapshotCMD, "snapshotTools.png", "Take snapshot of viewport"
        )
    except Exception as e:
        import traceback
    ...
    try:
        unrealToolsButton = slapFactoryShelf.shelfButton(
            blankCMD, "unrealTools.png", "Unreal Tools"
        )
        ...
        slapFactoryShelf.addMenu(
            unrealToolsButton, "Publish Shot To Unreal", publishShotForUnrealCMD
        )
        slapFactoryShelf.addMenu(
            unrealToolsButton, "Publish Layout Scene Description", USDSceneImportExportUIMayaCMD
        )
...
def buildSagaShelf():
    ...
    # For backward compatibility, call the original function
    makeShelfAndButtons()
